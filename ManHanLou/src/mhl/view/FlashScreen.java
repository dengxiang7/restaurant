 ...
 *是Graphics类中的一个方法：drawImage(Image img, int x, int y, int width, int height, ImageObserver observer) 
 *绘制指定图像中已缩放到适合指定矩形内部的图像;而ImageObserver用于在构造 Image 时，接收有关 Image 信息通知的异步更新接口。
 *也就是im是图像对象，260是x轴开始位置，10是y轴开始位置,280是宽大小，220是高大小，observer - 转换了更多图像时要通知的对象。
 *
 *getWidth得到是某个view的实际尺寸
 *getWidth在OnCreat的时候得到的是0..很奇怪, 我找到了原因: 当一个view对象创建时，android并不知道其大小，所以getWidth()和   getHeight()返回的结果是0，真正大小是在计算布局时才会计算，所以会发现一个有趣的事，即在onDraw( ) 却能取得长宽的原因。           那怎么在onCreat  的时候得到呢?   
 ...
 *
 *drawRect 就是绘制一个矩形 前面两个参数代表起始坐标， 也就是左上角 后面两个参数可以标识你想画的长度和宽度 也可以理解为右下角的坐标点。
 */
package mhl.view;
...
class paintcomponent1 extends JPanel implements Runnable
{
	...
	public void paint(Graphics g)
	{
		...
		Image image;
		image=Toolkit.getDefaultToolkit().getImage("image//flashscreen.jpg");
